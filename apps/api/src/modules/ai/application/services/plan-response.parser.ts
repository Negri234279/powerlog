...
export function parsePlanResponse(text: string, expectedEntryIds: readonly string[]): ParsedPlan {
    const result = planSchema.safeParse(parseJsonObject(text))
    if (!result.success) {
    ...
    for (const exercise of result.data.exercises) {
        if (!expected.has(exercise.entryId)) {
            throw new ModelAnswerRejection(`"${exercise.entryId}" is not one of the given entryId values`)
        }
        if (seen.has(exercise.entryId)) {
            throw new ModelAnswerRejection(`exercise "${exercise.entryId}" was programmed twice`)
        }
    ...
    if (missing.length > 0) throw new ModelAnswerRejection(`${missing.length} exercise(s) were left unprogrammed`)
