...
describe('ListChatConversationsHandler', () => {
    it('enriches each row with the other participant’s presence + identity', async () => {
        const presence = new FakePresenceReader().set(OTHER, {
            online: true,
            lastSeenAt: new Date('2026-05-01T10:00:00.000Z'),
        })
        const profiles = new FakeProfiles().set(OTHER, { username: 'athley', avatarUrl: 'https://a/1.png' })
        const handler = new ListChatConversationsHandler(new StubInbox([aRow()]), presence, profiles)
        ...
        expect(row?.otherParticipant).toEqual({ username: 'athley', avatarUrl: 'https://a/1.png' })
    })

    it('falls back to offline / empty identity when the sources cannot be read', async () => {
        ...
        // No profile seeded → read returns null → empty identity.
        const handler = new ListChatConversationsHandler(
            new StubInbox([aRow()]),
            new BrokenPresence(),
            new FakeProfiles(),
        )
        ...
        expect(row?.otherParticipant).toEqual({ username: '', avatarUrl: null })
    })
})
