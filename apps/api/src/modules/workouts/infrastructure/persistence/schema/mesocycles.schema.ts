...
/**
 * `mesocycles` — a multi-week training block. `owner_id` is a SOFT reference to
 * the auth `users` (no cross-module FK); cleanup on account deletion is driven by
 * an integration event. `start_date` anchors week 1 for session generation.
 */
