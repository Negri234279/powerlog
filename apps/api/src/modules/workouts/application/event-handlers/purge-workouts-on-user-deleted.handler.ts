 ...
 * removes the sessions, templates and mesocycles the user owns (their nested
 * rows cascade via the in-module FKs). Sessions a coach planned for an athlete
 * are owned by the athlete, so a coach leaving doesn't wipe their athletes' work.
 * Idempotent.
 */
...
export class PurgeWorkoutsOnUserDeleted implements IEventHandler<UserDeletedIntegrationEvent> {
    ...
    async handle(event: UserDeletedIntegrationEvent): Promise<void> {
        ...
        await this.mesocycles.deleteAllByOwner(event.userId)
    }
}
