            ...
            <div className="inset-hi rounded-[calc(2rem-0.375rem)] bg-surface p-6 md:p-8">
                <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                        <p className="font-mono text-eyebrow uppercase text-text-faint">{t('emailEyebrow')}</p>
                        <h2 className="mt-2 font-display text-h3 text-text">{t('verifyEmail')}</h2>
                        <p className="mt-2 max-w-lg text-body text-text-dim">
                            {t.rich('verifyEmailBody', {
                                em: (chunks) => <span className="text-text">{chunks}</span>,
                                email: me.email,
                            })}
                        </p>

                        {sent ? (
                            <p className="mt-4 text-sm text-pr">{t('verificationSent')}</p>
                        ) : (
                            <TrackedButton
